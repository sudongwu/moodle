<?php
$string['pluginname'] = '教务选课同步';
$string['term'] = '所在学期';
$string['courseinfo'] = '开课学院-课程名称-选课人数';
$string['jiaowu'] = '允许教务选课';
$string['role'] = '缺省分配的角色';
$string['enrolperiod'] = '保持选课时长';
$string['enrolstartdate'] = '开始时间';
$string['enrolenddate'] = '结束时间';
$string['longtimenosee'] = '多久不活动就撤销选课';
$string['sendcoursewelcomemessage'] = '发送课程的欢迎信息';
$string['customwelcomemessage'] = '自定义欢迎信息';
$string['cohortonly'] = '只有群成员';
$string['enrolme'] = '加入课程';

$string['jiaowu_error_notfind'] = '未有账号,请通知学生登录平台';
$string['jiaowu_error_exit'] = '选课退选';

$string['editselectedusers'] = '编辑选中用户的选课';
$string['deleteselectedusers'] = '删除被选中用户的选课';
$string['confirmbulkdeleteenrolment'] = '您确定删除这些用户的选课吗？';
$string['addgroup'] = '添加到组';
$string['notfound'] = '暂未在教务系统中获取到您的课程信息！';
$string['syn_success'] = '更新成功！';
$string['error_title'] = '同步异常列表！';
$string['reupdate'] = '重新执行选课';

$string['stuid'] = '学号';
$string['stuname'] = '姓名';
$string['stuemail'] = '电子邮箱地址';
$string['studept'] = '系别';
$string['role'] = '角色';
$string['error_reason'] = '异常原因';


