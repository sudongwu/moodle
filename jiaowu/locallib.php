 ...
 * @since  Moodle 3.0
 */
function enrol_jiaowu_check_group_enrolment_key($courseid, $enrolpassword) {
    global $DB;
...
class enrol_jiaowu_enrol_form extends moodleform {
    protected $instance;
    ...
    public function definition() {
        ...
        if ($instance->password) {
            ...
            $mform->addElement('password', 'enrolpassword', get_string('password', 'enrol_jiaowu'),
                    array('id' => 'enrolpassword_'.$instance->id));
            $context = context_course::instance($this->instance->courseid);
            $keyholders = get_users_by_capability($context, 'enrol/jiaowu:holdkey', user_picture::fields('u'));
            $keyholdercount = 0;
            foreach ($keyholders as $keyholder) {
                $keyholdercount++;
                if ($keyholdercount === 1) {
                    $mform->addElement('static', 'keyholder', '', get_string('keyholder', 'enrol_jiaowu'));
                }
            ...
            $mform->addElement('static', 'nokey', '', get_string('nopassword', 'enrol_jiaowu'));
        }

        $this->add_action_buttons(false, get_string('enrolme', 'enrol_jiaowu'));
        ...
        if ($instance->password) {
            if ($data['enrolpassword'] !== $instance->password) {
                if ($instance->customint1) {
                    ...
                    if (!enrol_jiaowu_check_group_enrolment_key($instance->courseid, $data['enrolpassword'])) {
                        // We can not hint because there are probably multiple passwords.
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_jiaowu');
                    }
                    ...
                    if ($plugin->get_config('showhint')) {
                        ...
                        $errors['enrolpassword'] = get_string('passwordinvalidhint', 'enrol_jiaowu', $hint);
                    } else {
                        $errors['enrolpassword'] = get_string('passwordinvalid', 'enrol_jiaowu');
                    }
                }
