 ...
 * @since     Moodle 2.6
 */
class enrol_jiaowu_external extends external_api {
    ...
    public static function get_instance_info($instanceid) {
        ...
        $enrolplugin = enrol_get_plugin('jiaowu');
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        $enrol = enrol_get_plugin('jiaowu');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_jiaowu');
        }
            ...
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_jiaowu_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_jiaowu')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_jiaowu', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_jiaowu($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
